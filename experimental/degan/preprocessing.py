...
if __name__ == "__main__":
    ...
    train_df, test_df, s_no, min_len = segment_data(data, normalize=False) # No segment level normalization
    ...
    for id, id_df in temp:
        X_w = split_sequence(id_df["meter_reading"], window_size,centering=True) # We will perform centering as
        # suggested by the paper
        X_train.extend(X_w)
        seg_count += 1
    X_train = np.array(X_train)
    X_train = X_train.reshape(len(X_train), 1, -1)  # (batch, channel, window)
